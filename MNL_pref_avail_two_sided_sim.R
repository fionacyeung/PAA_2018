...
setwd("C:\\UCLA\\thesis_ideas\\PhD_thesis\\multinomial_logit_model")

# source("one_to_one_matching_CP_x0_z0MR.R")
...
ff = ~ b1nodematch("f1") + b2nodematch("f1") +
  b1absdiff("f1",1) + b2absdiff("f1",1) 
...
# beta_wt = c(0.5,0.5,1)
# beta_mt = c(0.5,0.5,1)
beta_wt = c(0.5,0.5,1,1,1.5,1.5)
beta_mt = c(0.5,0.5,1,1,1.5,1.5)
...
for (n in N){
  ...
  for (b in 1:B){
    ...
    # X = matrix(rep(1:4,times=nw/4),ncol=1)
    X = matrix(NA, nrow=nw, ncol=1)
    proportion_w = c(0.2,0.3,0.3,0.2)
    # proportion_w = c(0.3,0.2,0.2,0.3)
    suprt_w = c(0, cumsum(proportion_w))
    ru_w = runif(nw)
    for (ww in 2:length(suprt_w)) {
      idx1 = which(ru_w > suprt_w[ww-1] & ru_w <= suprt_w[ww])
      X[idx1] = ww-1
    }
    colnames(X) = "f1"
    # Z = matrix(rep(0:1,times=nm/2),ncol=1)
    # Z = matrix(rep(1:4,times=nm/4),ncol=1)
    Z = matrix(NA, nrow=nm, ncol=1)
    proportion_m = c(0.3,0.2,0.2,0.3)
    # proportion_m = c(0.2,0.3,0.3,0.2)
    suprt_m = c(0, cumsum(proportion_m))
    ru_m = runif(nm)
    for (mm in 2:length(suprt_m)) {
      idx1 = which(ru_m > suprt_m[mm-1] & ru_m <= suprt_m[mm])
      Z[idx1] = mm-1
    }
    colnames(Z) = "f1"
    ...
    # increase the utilities for outside options 
    # (this results in beta_med: -5.723051 -0.5186954 3.253201 -5.723051 -0.5186955 3.253201
    # when the truth is 0.5, 0.5, 1, 0.5, 0.5, 1)
    # eta0 = 2.25*eta0
    # zeta0 = 2.25*zeta0
    ...
    # here, immediately below the mu, Xdata, Zdata have been generated above depending on the chosen parameter values for the preferences
    # these values were given by beta_wt = c(0.5,0.5,1) and beta_mt = c(0.5,0.5,1)
    # set the number of Monte Carlo iterations above to, for example, B = 200
    # set the size of the total (male+female) population above to, for example, N = c(200) or, concatenating, N <- c(20,50,100,200,500,1000,2000) 
    # Compute MLE based on an observed matching
    out <- fitrpm_R_CP(ff, mu, Xdata, Zdata, theta_0, control=control)
    
    # this gathers the estimated betas into a matrix
...
if (symmetric) {
  ...
  male_idx = (beta_all_df$beta %in% paste0("beta.",7:12))
  ...
  beta_all_df$beta[beta_all_df$beta == "beta.7"] = "beta.1"
  beta_all_df$beta[beta_all_df$beta == "beta.8"] = "beta.2"
  beta_all_df$beta[beta_all_df$beta == "beta.9"] = "beta.3"
  beta_all_df$beta[beta_all_df$beta == "beta.10"] = "beta.4"
  beta_all_df$beta[beta_all_df$beta == "beta.11"] = "beta.5"
  beta_all_df$beta[beta_all_df$beta == "beta.12"] = "beta.6"
  # see http://r.789695.n4.nabble.com/ggplot2-facet-grid-with-different-vertical-lines-on-each-facet-td3034783.html and
  # https://stackoverflow.com/questions/11846295/how-to-add-different-lines-for-facets
  # hline.dat = data.frame(beta=paste0("beta.",rep(1:3,times=2)), Coeff=beta_wt, agent.type=c(rep("female",3), rep("male",3)))
  hline.dat = data.frame(beta=paste0("beta.",rep(1:length(beta_w),times=2)), Coeff=beta_wt, agent.type=c(rep("female",6), rep("male",6)))
